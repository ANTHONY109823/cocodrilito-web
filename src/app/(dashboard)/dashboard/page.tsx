...
export default function DashboardPage() {
  ...
  const [chartScores, setChartScores] = useState<number[]>([55, 70, 60, 80, 75, 90, 85])
  ...
  const loadAll = async () => {
    try {
      ...
      setStats(s.data)
      setLatest(l.data)
      if (l.data?.score != null) {
        setChartScores((prev) => [...prev.slice(1), l.data.score])
      }
    } catch {
      /* ignore */
    } finally {
    ...
    <div className="mx-auto max-w-6xl space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          {daysToExam > 0 && (
            <p className="mt-1 text-xs text-[#C9943A]">
              ⏳ {daysToExam} días hasta el examen de ascenso (est.)
            </p>
          )}
        </div>
      ...
      {loading ? (
        <p className="text-center text-[#6B8A75] py-12">Cargando estadísticas...</p>
      ) : (
        <>
          <div className="grid grid-cols-2 gap-2.5 lg:grid-cols-4">
            ...
            <div className="rounded-xl border border-[rgba(189,255,223,0.12)] bg-[#0D1A10] px-3.5 py-4">
              ...
              <div className="text-[22px] font-extrabold text-white">—</div>
              ...
              <div className="flex h-20 items-end gap-1.5">
                {chartScores.map((h, i) => (
                  ...
                  >
                    ...
                    />
                    <span className="text-[9px] text-[#6B8A75]">{BAR_DAYS[i]}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {user?.planType === 'Free' && (
            <div className="rounded-xl border border-[#C9943A]/30 bg-[#C9943A]/10 p-4 flex flex-wrap items-center justify-between gap-3">
              <div>
